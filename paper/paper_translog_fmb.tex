...
\citet{matasraymond2009} examines the productivity of Spanish urban
transport at the national level, without isolating the rail concession
fee; \citet{daraio2016} reviews non-parametric frontiers for
international metro systems but without a Barcelona case. The contribution
of this paper is not to replicate the existence of density economies, but
to isolate the technological component of the cost function from the
institutional component and to demonstrate microeconomically that the two
must be analysed separately. We calibrate a translog cost function
$C_{\text{tec}}(Y,W)$ over the line-year panel under the assumption of
expenditure minimisation, and treat the contractual items
(concession fee, leasing) as an additive non-functional component
$F_{\text{inst}}$. We then show that forcing $F_{\text{inst}}$ inside the
translog produces concavity failure in 48 per cent of observations and a
3.8$\times$ degradation in fit, providing direct microeconomic evidence
that this is not a technological cost.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{../outputs/figures/fig8_conceptual.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\linewidth]{../outputs/figures/fig1_cost_composition.pdf}
...
homogeneity. The technical trend is $\lambda=-0.005$. The full list is
reported in Appendix~\ref{app:priors}.
...
The total cost observed in FMB's accounts includes two items---the Ifercat
L9 concession fee and train leasing---that do not meet the assumptions
underpinning the translog cost function. A cost function arising from
expenditure minimisation under given input prices must satisfy Shephard's
\citeyearpar{shephard1953} properties: monotonicity, concavity in prices,
linear homogeneity and symmetry. These properties derive from duality
with a quasi-concave production function and from the optimisation
condition on the operator.

The Ifercat L9 concession fee violates this framework on three
simultaneous grounds: (i) it does not depend on outputs in the observed
period, being a multi-year contractual payment; (ii) it does not depend
on input prices, being independent of wages or electricity prices; (iii)
it is not the outcome of optimisation by FMB, which cannot minimise but
only accept the fee.
...
financial-lease contracts with the manufacturer, typically multi-year
agreements with a fixed payment schedule. Unlike a spot price of capital,
the contractual price does not respond to year-by-year decisions and
cannot be the subject of short-run optimisation by the operator. From
the standpoint of the cost-minimisation framework underlying the translog
function, this makes the leasing payment closer to a contractually fixed
transfer than to a variable input price. Including it in $C_{\text{tec}}$
would impute optimisation behaviour to a payment that, in fact, FMB
cannot optimise within the observed period. We therefore include it in
$F_{\text{inst}}$, while acknowledging in Section~\ref{sec:limit} that
this is a conservative treatment: under an alternative capital-cost
modelling, average cost would shift marginally but marginal cost would
remain that of $C_{\text{tec}}$.
...
\label{eq:additive}
\end{equation}
where $C_{\text{tec}}(Y, W)$ is the translog cost function defined in
\eqref{eq:translog}, which satisfies the standard microeconomic
properties, and $F_{\text{inst}} = \text{concession fee} + \text{train leasing}$
is an additive non-functional component, accounted for but not modelled
as technology. This formulation has precedents in the network regulation
literature \citep{joskow2008}, and specifically in transport:
\citet{gagnepainivaldi2002} apply an analogous separation in urban bus
concessions, and \citet{mizutaniuranishi2013} distinguish infrastructure
from operating costs in Japanese railways. \citet{wheat2015} similarly
argue that pooling structurally heterogeneous cost components into a
single translog specification distorts the inferred returns to scale;
our additive separation extends this logic to institutional
heterogeneity.
...
The cost function specified in \eqref{eq:translog} is estimated at the
line-year level of aggregation. This requires clarifying what input
substitution means at that scale, since the technology of urban rail
exhibits a layered structure.

At the train-trip level, the production technology is essentially
Leontief: a conventional train requires exactly one driver, a fixed
electrical consumption determined by the rolling stock, and a fixed
maintenance intensity per car-km. Five drivers cannot substitute for one,
and a kilowatt-hour cannot replace a driver. This Leontief structure at
the micro level is a feature of urban-rail engineering recognised in the
literature \citep{small2007}.

Substitution emerges at the line-year level because the operator
aggregates many train-trips and chooses between alternative technologies,
vintages and policies. Three margins are economically relevant and
empirically active in FMB during 2023--2025:
...
These three margins generate the substitutability that the translog cost
function captures empirically. They are narrow---the calibrated
Allen--Uzawa elasticities of substitution of approximately 0.4--0.6
reflect precisely this narrowness---but non-zero, and sufficient for the
substitution matrix to exhibit the curvature properties required by
Shephard duality.

A fourth potential margin---adjusting service frequency---is not a clean
substitution channel because it shifts unobserved service quality rather
than substituting factors at constant output. A reduction in frequency
that maintains passenger demand implies fuller trains, longer waiting
times, and higher congestion. These are dimensions of the service product
not captured by our outputs $Y_1=$ passengers and $Y_2=$ car-km. The
empirical urban-rail cost literature has consistently noted the absence
of explicit quality measures as a limitation of the field
\citep{farsi2006,daraio2016}. We adopt the standard convention of
treating $Y_2$ as a proxy that bundles frequency with car-km, while
acknowledging in Section~\ref{sec:limit} the implications of this
simplification for the interpretation of returns to density.

The Ifercat L9 concession fee is impermeable to all three genuine
substitution margins identified above. No automation decision, no fleet
renewal decision, no make-or-buy decision in maintenance alters the fee,
which is fixed by multi-year contract with the Generalitat. The
concavity failure documented in Section~\ref{sec:h3} when $F_{\text{inst}}$
is forced inside the translog is therefore not the result of a generic
departure from cost minimisation. It is the direct consequence of
including in a cost function a component that is impervious to the only
substitution channels operating in the technology. The additive
decomposition in \eqref{eq:additive} is the formal recognition of this
structural fact: substitutable components belong inside the cost function
and generate concavity; non-substitutable contractual transfers belong
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\linewidth]{../outputs/figures/fig2_rtd_by_line.pdf}
...
Figure~\ref{fig:mc-ac} reports the line averages.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\linewidth]{../outputs/figures/fig3_mc_vs_ac.pdf}
    \caption{Average cost (AC) versus marginal cost (MC) by line, mean of
    2023--2025, technological cost function $C_{\text{tec}}$.}
    \label{fig:mc-ac}
\end{figure}

Figure~\ref{fig:subsidy} shows the temporal evolution: the Boiteux optimal
subsidy is stable across the three years.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{../outputs/figures/fig4_subsidy_boiteux.pdf}
    \caption{Boiteux optimal subsidy by year, technological cost function
    $C_{\text{tec}}$.}
    \label{fig:subsidy}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\linewidth]{../outputs/figures/fig_additive_decomposition.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{../outputs/figures/fig_concavity_diagnostic.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\linewidth]{../outputs/figures/fig6_heterogeneity_L9.pdf}
    \caption{Heterogeneity L9/L10 vs.\ L1--L5: the gap is institutional.
    Stacked bars decompose the passenger-weighted average cost into
    $C_{\text{tec}}$ (technological) and $F_{\text{inst}}$
    (institutional).}
    \label{fig:hetero}
\end{figure}

Figure~\ref{fig:shares} reports the operating cost shares by year, which
remain stable around a labour-dominated structure (57--58 per cent),
with energy at 4.6--5.9 per cent.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\linewidth]{../outputs/figures/fig5_cost_shares_operating.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{../outputs/figures/fig7_tornado.pdf}
...
of this paper---large returns to density in the technological cost
component (RTD~$\approx 2.18$), marginal cost roughly half of average
cost (MC/AC~$\approx 0.46$), and an institutional component
$F_{\text{inst}}$ accounting for some 29 per cent of total declared cost
with zero contribution to marginal cost---do not depend on the
imputation key, nor on the inclusion of the partial 2025 observations,
nor on the specific prior values used in calibration. They do depend on
the additive separation between $C_{\text{tec}}$ and $F_{\text{inst}}$,
which we defend theoretically in Sections~\ref{sec:treatment} and
\ref{sec:margins} and empirically in Section~\ref{sec:h3}.
...
The European regulatory framework provides a benchmark against which to
assess current FMB accounting practice. Directive 2012/34/EU
\citep{directive2012} requires mandatory accounting separation between
infrastructure managers and operators, on the rationale that mixing
infrastructure access charges with operating costs distorts efficiency
signals and biases regulatory comparisons. While urban metro systems
fall outside the formal scope of the directive, its underlying economic
logic applies with even greater force to FMB: the Ifercat L9 concession
fee represents nearly a quarter of total declared cost, larger than the
entire energy bill, and is paid to a separate public entity (Ifercat)
that owns the infrastructure. The fact that FMB consolidated accounts
present this charge as a current operating expense, alongside labour
and energy, is therefore at odds with the regulatory good practice that
the directive codifies for the broader rail sector. The microeconomic
demonstration in this paper---that $F_{\text{inst}}$ cannot be
represented within a cost function with standard microeconomic
properties---provides the formal foundation for what the directive
prescribes on regulatory grounds. The two arguments converge:
$F_{\text{inst}}$ must be reported separately for efficiency assessments
to be meaningful, regardless of whether formal compliance with Directive
2012/34/EU is required.

\subsection{Policy recommendations}

\paragraph{Accounting separation aligned with Directive 2012/34/EU.} The
FMB consolidated accounts present the Ifercat L9 concession fee as a
current operating expense, alongside labour and energy. The microeconomic
analysis presented here shows that this presentation is not innocuous:
it mixes a contractual infrastructure transfer with technological inputs,
biasing both operating-efficiency indicators and any consolidated
benchmarking against peer metros. The minimal regulatory recommendation
is therefore to report the concession fee separately---in line with the
accounting-separation principle codified in Directive 2012/34/EU for the
broader rail sector---so that downstream regulatory and academic users
can recover the technological cost function and assess operating
efficiency on a clean basis.

\paragraph{Reform of the concession fee: a capacity charge rather than a
fixed multi-year fee.} The current fee is largely independent of effective
use of L9/L10 capacity. Replacing it, in whole or in part, with a
capacity charge indexed to car-km or train-paths delivered would align
the cost perceived by the operator with the actual utilisation of the
infrastructure, generating efficiency signals consistent with network
regulation \citep{joskow2008,gomezlobo2014}. A hybrid scheme with a
fixed and a variable component is a natural intermediate option.

\paragraph{Review of the concession fee schedule in light of asset
useful life.} The underlying infrastructure has an expected useful life
in excess of 75 years; the current schedule front-loads cost recovery
in a way that puts disproportionate pressure on FMB's annual P\&L.
Reprofiling the schedule over a horizon closer to the asset life would
reduce the share of $F_{\text{inst}}$ in current cost without altering
the consolidated public debt position.

\paragraph{Train leasing: dynamic vs.\ static efficiency.} Leasing
delivers dynamic efficiency through quicker fleet renewal and
risk-sharing with the manufacturer, at the static cost of a rigid
recurrent expense growing nominally with the fleet. Greater transparency
on the contractual composition of the leasing payments---in particular,
on the split between capital recovery and service---would allow a more
informed regulatory debate.

\paragraph{Lessons for other cities.} Urban systems that finance
infrastructure through off-budget public debt repaid via long-term
availability fees inherit the same regulatory difficulty as Barcelona's
L9/L10. The Ifercat architecture is, in this respect, an instructive
case study: efficient on the financing side, but consequential for the
interpretation of operating cost on the service side.
...
\item \textbf{Service quality.} Frequency, mean waiting time, occupancy
and reliability are dimensions of the product that the operator can
adjust simultaneously with inputs. In our model, the implicit assumption
is that quality moves proportionally with car-km ($Y_2$). If FMB were
to reduce frequency to save inputs, this would appear as lower $Y_2$
with lower costs, consistent with a regular cost function, when it would
in fact reflect uncaptured quality degradation. The reported returns
to density should therefore be interpreted as conditional on
approximately constant service quality. As argued in
Section~\ref{sec:margins}, this limitation does not affect the central
conclusion on $F_{\text{inst}}$: the concession fee is impermeable to
all three genuine substitution margins, and its exclusion from
$C_{\text{tec}}$ is required by the substitution structure of the
technology, not by the constant-quality assumption. Future work with
explicit quality outputs would refine the cost-function estimates
without overturning the decomposition argument.
\item \textbf{Temporal pattern of $F_{\text{inst}}$.} The $F_{\text{inst}}$
share of total cost falls from 30.2 per cent in 2023 and 30.7 per cent
in 2024 to 26.8 per cent in the 2025 annualised figures. We cannot
distinguish between (a) genuine reductions in the concession fee or
leasing payments in the second half of 2025, (b) larger growth in
$C_{\text{tec}}$ components in 2025 that proportionally reduces the
institutional share, or (c) artefacts of the $\times 12/9$ annualisation
for partial-year data. The substantive conclusions on returns to
density and the additive decomposition are unaffected, since they hold
uniformly across years.
\end{itemize}

\subsection{Anticipated objections and responses}

\begin{table}[H]
\centering
\caption{Anticipated objections and responses.}
\label{tab:defensive}
\footnotesize
...
\midrule
``Why not treat train leasing as capital cost with depreciation?'' &
Train leasing is included in $F_{\text{inst}}$ as a conservative choice.
If capitalised with linear depreciation, AC would change marginally but
MC would remain equal to that of $C_{\text{tec}}$ because capital cost
is fixed in the short run. The marginal-cost conclusions are therefore
robust. \\
\midrule
``The additive decomposition is an accounting patch, not an economic
model.'' & The decomposition is grounded in network-regulation theory
\citep{joskow2008} and applied in transport
\citep{gagnepainivaldi2002,mizutaniuranishi2013}. The concavity failure
when $F_{\text{inst}}$ is forced inside the translog
(Section~\ref{sec:h3}) is empirical confirmation that the separation is
theoretically required, not a contrivance. \\
\midrule
``Why not use a translog with quasi-fixed inputs?'' & Variable-cost
models with quasi-fixed factors \citep{cavesswanson1981} could in
principle apply, but require long time series for partial-adjustment
identification. With $T=3$, this is unfeasible. The additive
decomposition is the only viable identification strategy that preserves
microeconomic properties. \\
\midrule
``Service quality is not modelled as an output.'' & Acknowledged as a
limitation (Section~\ref{sec:limit}). The omission does not affect the
decomposition argument: the concession fee is impermeable to the genuine
substitution margins identified in Section~\ref{sec:margins} (automation,
fleet renewal, maintenance make-or-buy), and its exclusion from
$C_{\text{tec}}$ is required by the substitution structure of the
technology, not by the constant-quality assumption. \\
\bottomrule
\end{tabularx}
\end{table}
...
\section{Full list of literature priors}
\label{app:priors}

\begin{table}[H]
\centering
\caption{Priors and references.}
\footnotesize
\begin{tabularx}{\linewidth}{lrX}
\toprule
Parameter & Value & Reference and justification \\
\midrule
$\sigma_{LE}$ & 0.40 & \citet{wheat2015}: limited substitution between
labour and energy in urban rail. \\
$\sigma_{LO}$ & 0.60 & \citet{wheat2015}: outsourcing of services
softens the labour factor. \\
$\sigma_{EO}$ & 0.50 & \citet{wheat2015}: energy and external services
are weakly complementary. \\
$\beta_1$ & 0.45 & \citet{savage1997}: U.S.\ metros,
$\beta_1\in[0.40, 0.50]$. \\
$\beta_2$ & 0.25 & \citet{savage1997}, \citet{graham2008}:
cost--car-km elasticity. \\
$\delta_{11},\delta_{22}$ & 0.05 & Modest curvature; function nearly
log-linear in outputs. \\
$\delta_{12}$ & $-0.05$ & Weakly complementary outputs. \\
$\rho_{LY_1}$ & $-0.04$ & More demand $\rightarrow$ smaller labour
share. \\
$\rho_{LY_2}$ & $+0.02$ & More service $\rightarrow$ slightly larger
labour share. \\
$\rho_{EY_1}$ & $+0.01$ & Adjusted from 0.03 for frequency rigidity
(collective agreements). \\
$\rho_{EY_2}$ & $+0.01$ & More car-km $\rightarrow$ slightly more
traction. \\
$\lambda$ & $-0.005$ & Modest negative technical trend. \\
\bottomrule
\end{tabularx}
\end{table}
